...
public class RoomDirectoryLookupHandler extends AuthenticatedServerApiHandler {

    public RoomDirectoryLookupHandler(GridifyServer g) {
        super(g);
    }

    @Override
    protected void handle(ServerSession session, Exchange ex) {
        String roomAlias = ex.getQueryParameter("room_alias");
        RoomLookup lookup = session.lookupRoomAlias(roomAlias)
                .orElseThrow(() -> new ObjectNotFoundException("Room Alias", roomAlias));

        JsonObject response = new JsonObject();
        response.addProperty("room_id", lookup.getId());
        response.add("servers", GsonUtil.asArray(lookup.getServers()));
